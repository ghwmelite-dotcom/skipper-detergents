...
beforeEach(async () => {
  ...
  await seedProducts(env.DB, [
    { id: 'p1', name: 'Skipper Liquid 2L', slug: 'skipper-liquid-2l', description: 'Liquid detergent', category_id: 'c1', brand: 'Skipper', unit_price: 45, stock_quantity: 100, is_featured: 1, is_bulk_available: 1, tags: 'liquid,detergent' },
    { id: 'p2', name: 'Ariel 3kg', slug: 'ariel-3kg', description: 'Ariel powder', category_id: 'c1', brand: 'Ariel', unit_price: 62, stock_quantity: 80, tags: 'powder,ariel' },
    { id: 'p3', name: 'Softcare 10-Pack', slug: 'softcare-10-pack', description: 'Toilet rolls', category_id: 'c2', brand: 'Softcare', unit_price: 35, stock_quantity: 200, is_featured: 1, tags: 'toilet-roll' },
    { id: 'p4', name: 'Inactive Product', slug: 'inactive', description: 'Hidden', category_id: 'c1', brand: 'X', unit_price: 10, is_active: 0 },
  ]);
...
describe('listProducts', () => {
  ...
  it('filters by category slug', async () => {
    const { products, total } = await listProducts(env.DB, { page: 1, per_page: 20, sort: 'newest', bulk_only: false, category: 'toilet-paper' });
    expect(total).toBe(1);
    expect(products[0]?.id).toBe('p3');
  });

  it('filters by brand', async () => {
    const { products } = await listProducts(env.DB, { page: 1, per_page: 20, sort: 'newest', bulk_only: false, brand: 'Ariel' });
    expect(products.map((p) => p.id)).toEqual(['p2']);
  });

  it('filters bulk_only', async () => {
    const { products } = await listProducts(env.DB, { page: 1, per_page: 20, sort: 'newest', bulk_only: true });
  ...
  it('sorts by price_asc', async () => {
    const { products } = await listProducts(env.DB, { page: 1, per_page: 20, sort: 'price_asc', bulk_only: false });
    expect(products.map((p) => p.unit_price)).toEqual([35, 45, 62]);
  });

  it('respects min_price and max_price', async () => {
    const { products } = await listProducts(env.DB, { page: 1, per_page: 20, sort: 'newest', bulk_only: false, min_price: 40, max_price: 50 });
    expect(products.map((p) => p.id)).toEqual(['p1']);
  });
});
